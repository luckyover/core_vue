IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[dbo].[SPC_M006_ACT2]') AND type in (N'P', N'PC'))
DROP PROCEDURE SPC_M006_ACT2
GO
SET ANSI_NULLS ON
GO
...
GO
-- +--TEST--+
-- EXEC SPC_M006_ACT2 1,'809','M001','172.19.0.1';
--****************************************************************************************
--* 											
--*  処理概要/process overview	:	Delete a supplier
--*  
...
--*　更新内容/update content		:   
--****************************************************************************************
CREATE PROCEDURE SPC_M006_ACT2
	@P_supplier_cd					INT				= 0
...
BEGIN
	...
	-- TRY CATCH EXCEPTION
	BEGIN TRY
		IF EXISTS(SELECT 1 FROM M006 WHERE supplier_cd = @P_supplier_cd)
		BEGIN
			UPDATE M006 SET
